...
int time_to_str(time_t time, char* buf, int size, const char* format) {
...
int time_to_str(time_t time, char* buf, int size) {
    return time_to_str(time, buf, size, "%04d-%02d-%02d %02d:%02d:%02d");
}
...
char* _now_to_str() {
    time_to_str(time(0), time_str_buf, sizeof(time_str_buf), "%04d-%02d-%02d %02d_%02d_%02d");
    return time_str_buf;
}
